...
*/
(function ($) {
    //declare 3 array variables for separately storing the list of cusRegionL1, cusRegionL2 and offeringsLevel3
    var cusRegionL1 = [];
    var cusRegionL2 = [];
    var cusRegionL3 = [];
    var cusRegionSelectedItems = 0;
    ...
    $.fn.cusRegiontreeview = function (settings) {
        var dfop =
            {
                method: "POST",
                datatype: "json",
        ...
        $("#cusRegionTreeClearSelect").click(nodeclick);
        //if identifierForCusRegTree is true, it's async tree, expland the root node(all)
        if (identifierForCusRegTree) {
            $("#cusRegionTree .bbit-tree-ec-icon").click();
        }
        ...
        function buildnode(nd, ht, deep, path, isend) {   //------------------data[i]:childnodes     ht:html,   0,  (0-2), (2)
            ...
            //added 3/26
            //identifierForCusRegTree is used to recognize whether it's search tree or async tree
            var level = deep;
            if (!identifierForCusRegTree) {
                ...
                if ($("#cusRegionTree").width() < 340) {
                    for (var i = 0; i < deep; i++) {
                        oldWidth1 += 50;
                        oldWidth2 += 50;
                        searchTextbox += 50;
                    }
                    $("#cusRegionTree").width(oldWidth1);
                    $("#closecusRegionTreeDiv").width(oldWidth2);
                    $("#searchcusRegionTreeDiv").width(oldWidth2);
                    $("#cusRegionTrMatchTextbox").width(searchTextbox);
                    $("#cusRegionNamesList").width(oldWidth2);
            ...
            //------------------------------------------------------highlight cusRegmatchString--------------------------------------
            //if the text of node includes the value of search textbox of product tree, hightlight this part
            var reg = new RegExp(cusRegmatchString, "i");
            var hightLightValue = nd.Text.match(reg);
            var noHightLightValue = "";
            var nodeText = "";
            if (!hightLightValue == null || !hightLightValue == "") {
                var startIndex = nd.Text.toLocaleLowerCase().indexOf(cusRegmatchString.toLocaleLowerCase());
                //startIndex=0:the cusRegmatchString is at the begining
                if (startIndex == 0) {
                    noHightLightValue = nd.Text.substr(cusRegmatchString.length);
                ...
                else if (startIndex == nd.Text.length - cusRegmatchString.length) {
                ...
                //the cusRegmatchString occurs at the middle position
                else {
                    var startValue = nd.Text.substring(0, startIndex);
                    var endValue = nd.Text.substr(startIndex + cusRegmatchString.length);
            ...
            if (item.render && pstate != item.checkstate) {
                ...
                if (et.length == 1) {
                    ...
                    if (item.checkstate == 0) {
                        if (item.Level == 1) {
                            var pOfElement = IsInArrayOff(item.Value, cusRegionL1);
                            if (pOfElement > -1) {
                                cusRegionL1.splice(pOfElement, 1);
                            }
                        }
                        if (item.Level == 2) {
                            var pOfElement = IsInArrayOff(item.Value, cusRegionL2);
                            if (pOfElement > -1) {
                                cusRegionL2.splice(pOfElement, 1);
                            }
                        }
                        if (item.Level == 3) {
                            var pOfElement = IsInArrayOff(item.Value, cusRegionL3);
                            if (pOfElement > -1) {
                                cusRegionL3.splice(pOfElement, 1);
                            }
                        }
                    }
                    else {
                        if (item.Level == 1) {
                            var pOfElement1 = IsInArrayOff(item.Value, cusRegionL1);
                            if (pOfElement1 == -1) {
                                AddEleInArrayOff(item.Value, cusRegionL1);
                            }
                        }
                        if (item.Level == 2) {
                            var pOfElement2 = IsInArrayOff(item.Value, cusRegionL2);
                            var pOfElement1 = IsInArrayOff(item.parent.Value, cusRegionL1);
                            if (pOfElement2 == -1) {
                                AddEleInArrayOff(item.Value, cusRegionL2);
                            }
                            if (pOfElement1 == -1) {
                                AddEleInArrayOff(item.parent.Value, cusRegionL1);
                            }
                        }
                        if (item.Level == 3) {
                            var pOfElement3 = IsInArrayOff(item.Value, cusRegionL3);
                            var pOfElement2 = IsInArrayOff(item.parent.Value, cusRegionL2);
                            var pOfElement1 = IsInArrayOff(item.parent.parent.Value, cusRegionL1);
                            if (pOfElement3 == -1) {
                                AddEleInArrayOff(item.Value, cusRegionL3);
                            }
                            if (pOfElement2 == -1) {
                                AddEleInArrayOff(item.parent.Value, cusRegionL2);
                            }
                            if (pOfElement1 == -1) {
                                AddEleInArrayOff(item.parent.parent.Value, cusRegionL1);
                            }
                        }
                    }

                }
                $("#CusRegLevel1Hidden").val(cusRegionL1.join('^'));
                $("#CusRegLevel2Hidden").val(cusRegionL2.join('^'));
                $("#CusRegLevel3Hidden").val(cusRegionL3.join('^'));
            }
        }
        ...
        function nodeclick(e) {
            ...
            if (et.id == "cusRegionTreeClearSelect") {
                path = "0";
                item = getItem(path);
                var state = 0;
                item.itemstate = 0;
            ...
            if (et.tagName == "IMG" || (et.tagName == "SPAN" && nodeLevel > 1) || et.tagName == "FONT") {
                    ...
                    asnyloadc(item, true, function (data) {
                        ...
                        if (item.checkstate == 1 && item.Level == 2) {
                            $.each(datas, function (key, val) {
                                var isExist = IsInArrayOff(val.Text, cusRegionL3);
                                if (isExist == -1) {
                                    cusRegionL3.push(val.Text);
                                }
                            });
                            $("#CusRegLevel3Hidden").val(cusRegionL3.join('^'));
                        }
                ...
                //if expended, collapse it
                    ...
                    var countOfNodes = $("#cusRegionTree div[level='" + nodeLevel + "'").next(" ul:visible").length;
                ...
                else if ($(et).hasClass("bbit-tree-node-cb") || $(et).hasClass("bbit-tree-node-span") || et.tagName == "FONT") // click on checkbox
                {
                    ...
                    if (nodeLevel == 1) {
                        if (item.checkstate == 0) {
                            asnyloadc(item, true, function (data) {
                                var datas = $.parseJSON(data);
                                $.each(datas, function (key, val) {
                                    var isExist = IsInArrayOff(val.Text, cusRegionL2);
                                    if (isExist == -1) {
                                        cusRegionL2.push(val.Text);
                                    }
                                });
                                $("#CusRegLevel2Hidden").val(cusRegionL2.join('^'));
                            });
                    ...
                    $("#cusRegionTreeConfirmSelect").attr("disabled", "disabled");
                    $("#txtcusRegionTree").attr("value", "calculating items");
                    //--end

                    //get the position of product tree div
                    var position = $("#cusRegionTree").position();
                    var heightOfcalculatingDiv = $("#closecusRegionTreeDiv").height();
                    ...
                    $.post(postUrl, { TreeType: "customerRegiontree", IsCheckbox: true, PL1Name: productLevel1Name, PL2Name: productLevel2Name, PL3Name: productLevel3Name, PL4Name: productLevel4Name, PL5Name: productLevel5Name, Level: item.Level }, function (data1) {
                        var getData = $.parseJSON(data1);

                        //                        $.each(getData, function (key, val) {
                        //                            cusRegionselectedProducts = val.LeafsId;
                        //                            cusRegionSelectedItems = val.Leafs;
                        //                        });
                        //                        $("#txtcusRegionTree").attr("value", "Selected " + cusRegionSelectedItems + " item(s)");
                        //                        $("#txtcusRegionTreeHidden").attr("value", cusRegionselectedProducts);
                        //                        $("#cusRegionTreeConfirmSelect").removeAttr("disabled");
                        //                        $("#calculatingDiv").hide();
                        //                        
                        if (item.checkstate == 1) {
                            var productsArray = cusRegionselectedProducts.split(",");
                            $.each(getData, function (key, val) {
                            ...
                            var productLeafs = cusRegionselectedProducts.substring(0, cusRegionselectedProducts.length - 1);
                            var cusRegionSelectedItems = productLeafs.split(",").length;
                            if (cusRegionselectedProducts == '') {
                                cusRegionSelectedItems = 0;
                                $("#txtcusRegionTree").attr("value", "Selected all item");
                            }
                            else {
                                cusRegionSelectedItems = productLeafs.split(",").length;
                                $("#txtcusRegionTree").attr("value", "Selected " + cusRegionSelectedItems + " items");
                            }
                        ...
                        if (item.checkstate == 0) {
                            cusRegionselectedProducts = cusRegionselectedProducts.substring(0, cusRegionselectedProducts.length - 1);
                            var productsArray = cusRegionselectedProducts.split(",");
                            ...
                            if (productsArray.length == 0) {
                                ...
                                cusRegionselectedProducts = productsArray.join(",");
        ...
        function LoadLevel3ByLevel1(level1, action) {
            var requestUrl = window.location.protocol + "//" + window.location.host + pathDir + "/GetCustomerRegionLevel3ByLevel1.srv";
            $.ajax({
                url: requestUrl,
                type: "GET",
                data: "Level1="+level1,
                ...
                success: function (data) {
                    ...
                    if (action == "push") {
                        $.each(result, function (key, val) {
                            var isExist = IsInArrayOff(val, cusRegionL3);
                            if (isExist == -1) {
                                cusRegionL3.push(val);
                            }
                        });
                        $("#CusRegLevel3Hidden").val(cusRegionL3.join('^'));
                    }
        ...
        function asnybuild(nodes, deep, path, ul, pnode) {
            ...
            if (l > 0) {
                ...
                //                $("#cusRegionTree").height(oldHeight1);
                //                $("#closecusRegionTreeDiv").height(oldHeight2);
